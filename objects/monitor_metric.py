...
@base.NovaObjectRegistry.register
class StringMonitorMetric(base.NovaObject):
    # Version 1.0: Initial version
    # Version 1.1: Added NUMA support
    ...
    def obj_make_compatible(self, primitive, target_version):
        super(StringMonitorMetric, self).obj_make_compatible(primitive,
                                                             target_version)
        target_version = versionutils.convert_version_to_tuple(target_version)
        if target_version < (1, 1) and 'numa_membw_values' in primitive:
            del primitive['numa_membw_values']
...
@base.NovaObjectRegistry.register
class SMonitorMetricList(base.ObjectListBase, base.NovaObject):
    # Version 1.0: Initial version
    # Version 1.1: MonitorMetric version 1.1
    VERSION = '1.1'

    fields = {
        'objects': fields.ListOfObjectsField('StringMonitorMetric'),
    }
    ...
    def from_json(cls, metrics):
        ...
        for metric in metrics:
            if ('value' in metric
                and metric['name'] in FIELDS_REQUIRING_CONVERSION):
                metric['value'] = metric['value'] * 100
            metric_list.append(StringMonitorMetric(**metric))

        return SMonitorMetricList(objects=metric_list)
