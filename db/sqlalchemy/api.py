...
def _instance_purge(context, dry_run, instance):
    LOG.debug('<Instance(instance_uuid=%s,'
              ' name=%s, deleted_at=%s)>'
              % (instance['uuid'], instance['name'],
                 instance['deleted_at']))
    related_instance_models = [models.SecurityGroupInstanceAssociation,
                               models.InstanceInfoCache,
                               models.InstanceMetadata,
                               models.InstanceFault,
                               models.InstanceExtra,
                               models.InstanceSystemMetadata,
                               models.InstanceAction,
                               models.BlockDeviceMapping,
                               models.Console,
                               models.PciDevice,
                               models.Migration,
                               models.VirtualInterface]

    for related_instance_model in related_instance_models:
        related_model_query = model_query(context,
                                          related_instance_model).\
                                filter_by(instance_uuid=instance['uuid'])
        results = related_model_query.all()
        for result in results:
            LOG.debug('  <%s(id=%s)>'
                        % (type(result).__name__, result['id']))
            if isinstance(result, models.InstanceAction):
                # Entries from the instance_action_events table that have a
                # foreign key to the instance_actions table must be first
                # deleted prior to deleting the related field in
                # the instance_actions table
                instance_action_events_query = model_query(context,
                                                models.InstanceActionEvent).\
                                                filter_by(action_id=
                                                          result['id'])
                instance_action_events = instance_action_events_query.all()
                for instance_action_event in instance_action_events:
                    LOG.debug('    <InstanceActionEvent(id=%s)>' %
                              instance_action_event['id'])
                if not dry_run:
                    instance_action_events_query.delete()
        if not dry_run:
            related_model_query.delete()

    tags_query = model_query(context, models.Tag).\
                 filter_by(resource_id=instance['uuid'])
    tags = tags_query.all()
    for tag in tags:
        LOG.debug('  <Tag(tag=%s)>' % tag['tag'])
    if not dry_run:
        tags_query.delete()

    instance_by_uuid_query = model_query(context, models.Instance).\
            filter(models.Instance.uuid == instance['uuid'])
    if not dry_run:
        instance_by_uuid_query.delete()
...
def instances_purge_deleted(context, dry_run=False, older_than=90,
                            max_number=None):
    try:
        older_than = utils.validate_integer(older_than, "older_than")
        if max_number is not None:
            max_number = utils.validate_integer(max_number, "max_number",
                                                min_value=1)
    except exception.InvalidInput as e:
        raise exception.InvalidInput(message=e.format_message())

    purge_before_datetime = timeutils.utcnow() - \
    ...
    instance_query = model_query(context, models.Instance).\
                        ...
                        models.Instance.deleted_at < purge_before_datetime))
    q_offset = 0
    more_instances = True
    ...
    while more_instances:
        ...
        if max_number is not None:
            if deleted_instances >= max_number:
                break
            if limit > (max_number - deleted_instances):
                limit = max_number - deleted_instances
        for instance in instance_query.limit(limit).offset(q_offset):
            more_instances = True
            _instance_purge(context, dry_run, instance)
            deleted_instances += 1
        q_offset += CONF.osapi_max_limit
