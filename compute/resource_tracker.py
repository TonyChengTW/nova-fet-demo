...
class ResourceTracker(object):
    ...
    def instance_claim(self, context, instance,
                       limits=None, filter_properties=None):
    ...
    def _get_host_metrics(self, context, nodename):
        ...
        """
        imetrics = objects.MonitorMetricList()
        smetrics = objects.SMonitorMetricList()
        metrics_info = {}
        for monitor in self.monitors:
            try:
                if 'bmc' in monitor.get_metric_names():
                    monitor.populate_metrics(smetrics)
                else:
                    monitor.populate_metrics(imetrics)
        ...
        metric_list.extend(smetrics.to_list())
        return metric_list
