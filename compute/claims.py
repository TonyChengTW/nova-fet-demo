...
class Claim(NopClaim):
    ...
    def __init__(self, context, instance, tracker, resources, pci_requests,
                 overhead=None, limits=None, filter_properties=None):
        ...
        self._claim_test(resources, limits, filter_properties)
    ...
    def _test_instance_group_policy(self, filter_properties):
        ...
        group = objects.InstanceGroup.get_by_hint(self.context, group_hint)
        if 'anti-affinity' in group.policies:
            group_hosts = group.get_hosts(exclude=[self.instance.uuid])
            if self.tracker.host in group_hosts:
            ...
            group_hosts = group.get_hosts(exclude=[self.instance.uuid])
            if group_hosts and self.tracker.host not in group_hosts:
